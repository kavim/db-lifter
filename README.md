...
Restoring large MySQL dumps (multi-GB) into Docker containers is slow and memory-intensive with naive approaches. `db-lifter` solves this by streaming the dump file directly into the container's `mysql` process through OS pipes — no intermediate buffers, no temp files, no wasted RAM.
...
```bash
git clone https://github.com/kevinmacielmedeiros/db-lifter.git
cd db-lifter
make build
```

The binary is compiled to `bin/db-lifter`. It is a static binary with no external dependencies — copy it anywhere on your `PATH`.

### Run directly

```bash
go run ./cmd/db-lifter -f dump.sql -c mysql-container -d mydb
```
...
```bash
db-lifter --recreate-database -f ./backup.sql -c mysql-dev -u root -p secret -d my_app
```

Import only into an existing empty database (no drop):

```bash
db-lifter -f ./backup.sql -c mysql-dev -d my_app
```
...
```bash
db-lifter --env .env -f /data/other-dump.sql
```

## Configuration via `.env` file

When you pass `--env`, `db-lifter` loads the specified file and maps its variables to the corresponding flags. Any flag explicitly set on the command line takes precedence.
...
```
.
├── cmd/db-lifter/          # CLI entry point (Cobra, signal handling)
│   └── main.go
├── internal/
...
| Target                | Description                                |
| --------------------- | ------------------------------------------ |
| `make build`          | Compile a static binary to `bin/db-lifter` |
